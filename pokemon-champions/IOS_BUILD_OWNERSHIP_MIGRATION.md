...
`buddy-brain` is the current owner of working BeMoreAgent iOS validation and TestFlight automation.
...
Current workflows in `buddy-brain`:
...
1. Freeze current workflow truth in `buddy-brain`
...
7. Then demote `buddy-brain` from canonical build owner
...
- `buddy-brain` no longer acts as canonical release owner
