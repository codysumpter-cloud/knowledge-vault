...
- **Log-First Triage:** 当代理报告 400/401/402 等 API 错误时，不要仅凭总结判断。立即 dump 原始日志（如 `tail /home/hermes/.hermes/logs/errors.log`）并使用 `curl` 直接测试 API 密钥和模型 ID，以区分是配置错误、密钥失效还是供应商波动。
- **Permission Boundary Checks:** 运行非 root 用户代理（如 `hermes`）时，警惕 `PermissionError (Errno 13)`。若代理尝试扫描父目录（如 `/root/.git`）以寻找项目根目录，会触发崩溃。必须显式设置 `cwd` 或确保代理在其权限范围内拥有有效的 `.git` 根。
...
- 上一次修复没有生效
- 你没有完全理解问题
- **当你发现自己在用 GUI 自动化（如 AppleScript/Vision, Browser Tools）进行重复尝试且没有进展时：**
- 这是一个极强的信号，表明目标系统正在执行主动拦截（如 Cloudflare Bot Detection）或操作系统权限拦截（如 macOS Sandbox/TCC）。
- **立即停止 GUI 猜测。** 不要尝试更换不同的自动化工具（例如从 AppleScript 换到 Agent-Browser），因为拦截通常发生在协议或指纹层。
- **强制路径切换：** 立即切换到 API 模式、CLI 模式，或诚实地告知用户自动化被拦截并请求用户手动执行一个特定步骤。
- **禁止循环尝试：** 连续 3 次以上尝试不同自动化工具但均未成功 $\rightarrow$ 判定为“自动化失效”，禁止继续尝试直至路径变更。
