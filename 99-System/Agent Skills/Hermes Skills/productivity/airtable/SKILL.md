...
curl -s "https://api.airtable.com/v0/$BASE_ID/$TABLE?maxRecords=5" \
  -H "Authorization: Bearer [REDACTED]" | python3 -m json.tool
```
...
while :; do
  ...
  RESP=$(curl -s "$URL" -H "Authorization: Bearer [REDACTED]")
...
1. **Confirm auth.** `curl -s -o /dev/null -w "%{http_code}\n" https://api.airtable.com/v0/meta/bases -H "Authorization: Bearer [REDACTED]"` — expect `200`.
