...
The skill does **not** claim to contain all of Wikipedia inside the repository. It provides a reproducible method for deriving concise notes from Wikipedia pages, dumps, and citations while preserving provenance, uncertainty, and refresh rules.
...
7. Recommend primary or authoritative sources when stakes are high.
...
- Verify live before using for: <dates, active roles, prices, policies, current events>
```
...
Bad notes look like copied encyclopedic paragraphs. Good notes look like reusable thinking tools.
...
- current public office holders or active conflicts
- professional health decisions
- legal advice
- financial advice
...
source_attribution:
  ...
  license_note: source license and attribution requirements must be preserved
```
...
stable:
  examples:
    ...
    - grammar concepts
  behavior: derived note may be reused with source metadata
...
- reminder to verify volatile facts live
